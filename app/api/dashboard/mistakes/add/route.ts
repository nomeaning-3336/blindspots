...
export async function POST(request: Request) {
  ...
  // Dedup by (user_id, starting_fen) — if the user has already added this
  // position, return the existing row instead of creating a duplicate.
  ...
  if (existing) {
    ...
    // If the user previously deleted it, revive it back to active.
    ...
    return NextResponse.json({ ok: true, mistakeId: row.id, deduped: true });
  }
  ...
  return NextResponse.json({ ok: true, mistakeId: (inserted as any)?.id ?? null, deduped: false });
}
