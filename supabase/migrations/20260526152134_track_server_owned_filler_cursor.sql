alter table public.user_blindspot_profile
  add column if not exists next_filler_cursor integer not null default 0;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'user_blindspot_profile_next_filler_cursor_check'
      and conrelid = 'public.user_blindspot_profile'::regclass
  ) then
    alter table public.user_blindspot_profile
      add constraint user_blindspot_profile_next_filler_cursor_check
      check (next_filler_cursor >= 0);
  end if;
end
$$;

comment on column public.user_blindspot_profile.next_filler_cursor is
  'Server-owned deterministic filler traversal cursor. Incremented only when a filler session is consumed or explicitly skipped.';
...
begin
  ...
  if v_session.selected_training_item_id is not null then
    select *
  ...
  set
    blindspots_elo = p_elo_after,
    rating_deviation = p_rating_deviation_after,
    total_sequences = coalesce(total_sequences, 0) + 1,
